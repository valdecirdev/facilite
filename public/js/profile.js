 ...
 */ 
    $("#form-usrFoto").submit(function(e) {
        if($("#usrFoto").val() !== ''){
            ...
            $.ajax({
                type: 'POST',
                url: "post/update_avatar", // Url do lado server que vai receber o arquivo
                data: form,
                processData: false,
                contentType: false,
                success: function (data) {
...
if(($('#id_servico_modal').val() != '')&&($('#id_servico_modal').val() != undefined)){
    ...
    function(data){
        var categoria = $('#des_categoria_servico option:selected');
        var modalidade = $('#des_modalidade_servico option:selected');
        $('#servicos-itens').append('<div class="col-12 clearfix" style="margin-bottom:5px"><input type="text" class="d-none id_servico" value="'+data+'"><div class="row clearfix"><p class="des_categoria_servico col-11" id="'+categoria.val()+'" style="width:auto; margin-left:-5px;font-weight:400; padding-right:25px;font-size:17px;margin-bottom:0px"><a href="/servico/'+data+'">'+categoria.text()+'</a></p><div class="btn-group col-1"><button type="button" class="btn btn-link dropdown-toggle" style="color:#5b5656" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></button><div class="dropdown-menu dropdown-menu-right"><button class="btn-editServico dropdown-item" type="button" style="cursor:pointer">Editar</button><button class="btn-delServico text-danger dropdown-item" type="button" style="cursor:pointer">Deletar</button></div></div></div><p class="col-12 desc des_descricao_servico" style="margin-left:-10px;margin-right:-10px; padding-left:5px;padding-bottom:1px;padding-top:10px;font-weight:300;font-size:16px;">'+descricao.val()+'</p><p class="desc" style="margin-left:-5px">Preço: R$ <span class="des_preco_servico">'+preco.val()+'</span><span class="des_modalidade_servico" id="<?php echo $modalidade->getIdModalidade(); ?>" style="margin-right:15px">'+' '+modalidade.text()+'</span>Disponibilidade: <span class="des_disponibilidade_servico">'+disponibilidade.val()+'</span></p><hr style="margin-left:-15px;margin-right:-15px"></div>');
